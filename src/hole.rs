...
impl<'a, T> Drop for Hole<'a, T> {
    fn drop(&mut self) {
        unsafe {
            // SAFETY: `elt` is no longer used
            let elt = ManuallyDrop::take(&mut self.elt);
...
impl<'a, 'b, T> HoleSwap<'a, 'b, T> {
    ...
    /// and move the hole to its previous position.
    ...
    pub fn swap_with(&mut self) {
        // SAFETY: `index` is a valid index in `data` and not a hole
        let parent = unsafe { self.hole.data.get_unchecked_mut(self.index) };
        mem::swap(parent, &mut self.hole.elt);
    }
}
